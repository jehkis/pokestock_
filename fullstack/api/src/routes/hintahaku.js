...
const Markkinahinta = require('../models/Markkinahinta');
...
// Etsii markkinahintaviitteesta kortin, jonka nimi alkaa annetulla nimella
...
router.get('/', async (req, res) => {
  try {
    ...
    if (numero && numero.trim()) {
      query.kortti_nimi = {
        $regex: new RegExp(`^${escapeRegex(nimi.trim())}\\b(?=.*${escapeRegex(numero.trim())})`, 'i'),
      };
    } else {
      query.kortti_nimi = { $regex: new RegExp(`^${escapeRegex(nimi.trim())}\\b`, 'i') };
    }

    let candidates = await Markkinahinta.find(query).limit(50).lean();
